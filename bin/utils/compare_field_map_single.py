...
def initialise_maus(config):
    configuration = Configuration.Configuration().\
                                          getConfigJSON(open(config, "r"))
...
def plot_hps(z_list, different_markers, flip_z = None):
    ...
    for hp in sorted(hp_z_positions.keys()):
        ...
        if z_pos > z_list[0] and z_pos < z_list[-1]:
            ...
            hp_color = 8 #hp_color_list[len(graph_list)%5]
...
def text_box(graph_list):
    text_scale = 1.0 #1.5
    legend = ROOT.TLegend(0.75, 0.41, 0.88, 0.89) # 0.83, 0.41, 0.99, 0.99)
    for graph in graph_list:
        legend.AddEntry(graph, graph.GetName(), "lp")
    legend.SetBorderSize(0)
    legend.Draw()
    TEXT_BOXES.append(legend)
    text_box = ROOT.TPaveText(0.75, 0.22, 0.88, 0.41, "NDC") # 0.83, 0.22, 0.99, 0.41, "NDC")
    ...
    text_box = ROOT.TPaveText(0.75, 0.12, 0.88, 0.22, "NDC") # 0.83, 0.12, 0.99, 0.22, "NDC")
    text_box.SetFillColor(0)
    text_box.SetBorderSize(0)
...
def plot_z_range(z_list, b_min_max, name, canvas):
    ...
    for r_pos, line_color in [(160., 8), (0., 1)]:
        ...
        hist, graph = xboa.common.make_root_graph("x="+str(r_pos/LENGTH)+" m", gz_list, "z [m]",
                                                  btot_list, "B_{z} [T]",
                                                  xmin=xmin, xmax=xmax,
                                                  ymin=ymin, ymax=ymax)
...
def show_primitives(root_obj):
    for an_object in root_obj.GetListOfPrimitives():
        print an_object.GetName()

def schematics_plot(my_dir):
    #initialise_maus("./config_9949.in")
    initialise_maus("./config_9885.in")
    file1 = ROOT.TFile.Open("./field_single/schematics_plot.root" )
    #canvas = ROOT.TCanvas("canvas", "canvas", 1200, 500)
    canvas = file1.Get("canvas")
    #canvas.Divide(1, 1, 0.01, 0.1)
    for an_object in  canvas.GetListOfPrimitives():
        name = an_object.GetName()
        if name == "canvas_1":
            canvas_plot = an_object
    for an_object in canvas_plot.GetListOfPrimitives():
        name = an_object.GetName()
        if name == "canvas_1_1":
            subcanvas = an_object
    try:
        show_primitives(subcanvas)
    except:
        print "No subcanvas"
    ...
    raw_input("Showing old plot.. press enter to continue... ")
    plot_z_range(range(13000, 21001, 1), [None, None], my_dir+"bfield_vs_z", canvas_plot.cd())
...
def main():
    """
    Make a plot of z, bz for 9885 field config, plot over previous field map
    """
