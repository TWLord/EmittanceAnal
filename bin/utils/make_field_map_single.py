...
def text_box(graph_list):
    text_scale = 1.0 #1.5
    legend = ROOT.TLegend(0.77, 0.41, 0.88, 0.89) # 0.83, 0.41, 0.99, 0.99)
    for graph in graph_list:
        legend.AddEntry(graph, graph.GetName(), "lp")
    legend.SetBorderSize(0)
    legend.Draw()
    TEXT_BOXES.append(legend)
    text_box = ROOT.TPaveText(0.77, 0.22, 0.88, 0.41, "NDC") # 0.83, 0.22, 0.99, 0.41, "NDC")
    ...
    text_box = ROOT.TPaveText(0.77, 0.12, 0.88, 0.22, "NDC") # 0.83, 0.12, 0.99, 0.22, "NDC")
    text_box.SetFillColor(0)
    text_box.SetBorderSize(0)
