...
class ConglomerateOne(object):
    ...
    def get_stats_list(self, hist_list): 
        ROOT.gStyle.SetOptStat(1100)
        stats_list = []
        print "Getting stats boxes from", len(hist_list), "histograms"
        tmpcanvas = ROOT.TCanvas("tmpcanvas")
        tmpcanvas.cd()
        for i in range(len(hist_list)):
            print i
            hist_list[i].ResetBit(ROOT.kCanDelete)
            hist_list[i].SetStats(1)
            h = hist_list[i].Draw()
            tmpcanvas.Update()
            #ROOT.gPad.Update()
            print hist_list[i].GetRMS()
            stats = hist_list[i].GetListOfFunctions().FindObject("stats").Clone("stats_"+self.uid())
    ...
    def redraw(self, canvas, hist_list, graph_list):
        ...
        do_stats = True
        if do_stats == True:
            #self.arrange_stats_boxes(hist_list)
            self.arrange_stats_boxes2(self.stats_list)
    ...
    def arrange_stats_boxes(self, hist_list):
        ydelta = 0.8/len(hist_list)
        stats_list = []
        for i, h in enumerate(hist_list):
            #h.ResetBit(ROOT.kCanDelete)
            #h.SetStats(1)
            #h.Draw()
            #ROOT.gPad.Update()
            print h.GetMean()
    ...
    def arrange_stats_boxes2(self, stats_list):
        ydelta = 0.8/len(stats_list)
        print "stats list:", len(stats_list)
        print 'ydelta', ydelta
        for i, stats in enumerate(stats_list):
            y_l = 0.1 + ydelta*i
            #stats.SetY1NDC(y_l)
            #stats.SetY2NDC(y_l+ydelta/1.2)
            print 'y_l', y_l
            print 'y_l+ydelta/2', y_l+ydelta/1.2
            stats.Draw()
            ROOT.gPad.Update()
    ...
    def murgle_histograms(self, canvas, hist_list, graph_list):
        ...
        self.defit(canvas, hist_list, graph_list)
        self.stats_list = self.get_stats_list(self.hist_list)
