...
class ConglomerateMerge(object):
    ...
    def do_mice_logo(self, do_isis, row, col):
        options = self.cong_options.options
        micetext = options["mice_logo"]
        preliminary = False
        ...
        if do_isis:
            ...
            text_box.AddText("ISIS Cycle")
            text_box.AddText("2017/02, 2017/03")
            text_box.AddText("2017-2.6")
        text_box.Draw()
        self.root_objects.append(text_box)
    ...
    def draw_cong(self, pad, cong, i, j):
        ...
        cong.murgle_many(pad, cong.hist_list, cong.graph_list, i,j)
            ...
            for j in range(self.cols):
                ...
                if cong.options["mice_logo"]:
                    if i == 0 and j == 0:
                        ...
                        #self.do_mice_logo(True, i, j) #i == 0 and j == self.cols-1)
        merge_canvas.cd()
        if source_x_axis:
            source_x_axis.Draw()
        if source_y_axis:
            source_y_axis.Draw()
        #self.do_legend_4() # big
        #self.do_legend_3() # small
        self.do_outer_box()
