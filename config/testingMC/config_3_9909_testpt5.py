...
def mc_file_names(run_number_list):
    ...
    for run in run_number_list:
        ...
        a_file = "/data/mice/phumhf/MC/MAUSv3.3.2/"+run+"v3/*_sim.root" 
...
def get_systematics_dir(emittance, suffix, absorber, analysis):
    ...
    a_dir = "output/2017-02-6-Systematics-"+vers+"/plots_Simulated_2017-2.7_"+str(emittance)+\
...
def get_analysis(run_list, name, tof01_min_max, maus_version, data_dir, emittance, p_bins, tkd_cut, tramlines_dp):
    ...
    analysis_variables = {
            ...
            "pt_us_low":0., # upstream transverse momentum cut lower bound
            "pt_us_high":10., # upstream transverse momentum cut lower bound
            "pt_ds_low":0., # upstream transverse momentum cut lower bound
            "pt_ds_high":10., # upstream transverse momentum cut lower bound
...
class Config(object):
    ...
    downstream_cuts["pt_us"] = False # Swapped pt cut
    downstream_cuts["pt_ds"] = False # Swapped pt cut
    ...
    mc_true_us_cuts["mc_pt_us"] = False 
    ...
    mc_true_ds_cuts["mc_pt_ds"] = True 

    cut_report  = [[], [], []]
    cut_report[0] = ["hline", "all events", "hline",]
    cut_report[0] += ["mc_stations_us", "mc_scifi_fiducial_us", "hline",]
    cut_report[0] += ["mc_p_us", "hline",]
    cut_report[0] += ["mc_pt_us", "hline",]
    ...
    cut_report[1] += ["mc_stations_ds", "mc_scifi_fiducial_ds", "mc_p_ds", "hline",]
    ...
    data_dir = "output/testingofficialMC/2017-02-6-v3-OfficialMC_fullpt5/"
    analyses = []


    analyses.append(get_analysis(["9909"],  "9909 2017-02-6 3-240 ABS-LH2",  [1.5, 2.75], src_dir, data_dir, 3, [[235, 245]], [190, 270], 24)) 
    ...
    number_of_spills = 50 # 5 # 100 # 8 # None # if set to an integer, limits the number of spills loaded for each sub-analysis
    preanalysis_number_of_spills = 20 # 5 # 20 # number of spills to analyse during "pre-analysis"
    analysis_number_of_spills = 20 # 20 # number of spills to analyse during each "analysis" step
