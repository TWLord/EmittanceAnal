...
def mc_file_names(datasets):
    ...
    for run in datasets: 
        run = str(run)
        a_file = "/data/mice/phumhf/analMC/"+run+"_systematics_v106/tkd_rot_plus/*/maus_reconstruction.root" 
...
class Config(object):
    ...
    data_dir = "output/testingsystematics/2017-02-6-c7_v106/"
    analyses = []


    analyses.append(get_analysis([9909],  "2017-02-6 3-240 ABS-LH2 tkd_rot_plus",  [1.5, 4.5], src_dir, data_dir, 3, [[235, 245]], [190, 270], 24)) 
    if analyses[0]["name"].find("tku_base") >= 0 :
        print "also running --- "
        print " tku_base_tkd_fiducial_radius"
        analyses.append(get_analysis([9909],  "2017-02-6 3-240 ABS-LH2 tku_base_tkd_fiducial_radius",  [1.5, 4.5], src_dir, data_dir, 3, [[235, 245]], [190, 270], 24))
        analyses[1]["tkd_fiducial_radius"] = 148. # r2 ~ pt/bz/c_light ~ pt [mm]

        print " tkd_chi2_threshold"
        analyses.append(get_analysis([9909],  "2017-02-6 3-240 ABS-LH2 tku_base_tkd_chi2_threshold",  [1.5, 4.5], src_dir, data_dir, 3, [[235, 245]], [190, 270], 24))
