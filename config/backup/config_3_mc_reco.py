...
def reco_file_names(run_number_list, maus):
    ...
    for run in run_number_list:
        ...
        a_file = "/data/mice/phumhf/ReconData/Mausv3.3.2/run_"+run+"/"+run+"_recon.root"
        #a_file = "/data/mice/phumhf/MC/MAUSv3.3.0/"+run+"/"+run+"_*1.root"
        #a_file = "/data/mice/phumhf/MC/MAUSv3.3.0/"+run+"/mausoutput/0/"+run+"_1000Spills_2.0NPEDcut_2.0NPEcut_1.root"
...
def get_analysis(run_list, name, tof01_min_max, maus_version, data_dir, emittance, p_bins, tkd_cut, tramlines_dp):
    ...
    analysis_variables = {
            ...
            "skip_corrections":True, #get_systematics_dir(emittance, "tku_base", "lH2_empty", "amplitude"),
...
class Config(object):
    ...
    number_of_spills = 5 #100 #None # if set to an integer, limits the number of spills loaded for each sub-analysis
