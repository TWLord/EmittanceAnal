...
class AngMomFields(AnalysisBase):
    def __init__(self, config, config_anal, data_loader):
        ...
        #sys.argv = []
        #self.geo = os.path.join(self.config.geometry_path, 'runnumber_'+self.config_anal["run_list"][0], 'ParentGeometryFile.dat')
        #sys.argv.append('dummy_arg')
        #sys.argv.append('--simulation_geometry_filename')
        #sys.argv.append(self.geo)
        #print 'Passing to maus..'
        #print sys.argv
        #self.initialise_maus()

    def initialise_maus(self):
    ...
    def birth(self):
        self.set_plot_dir("ang_mom_field_plots")
        self.get_var_list()
        ...
        for detector, virt_station_list in self.mc_stations.iteritems(): 
            ...
            for detector in self.reco_stations: 
                ...
                self.birth_var_two_d_hist("r_vs_Bz_map_at_"+detector+"_us_cut", detector, "r", "Bz_map", cuts = "upstream_cut", wild_x_axis=True, wild_y_axis=True)
                self.birth_var_two_d_hist("r_vs_Bz_map_at_"+detector+"_ds_cut", detector, "r", "Bz_map", cuts = "downstream_cut", wild_x_axis=True, wild_y_axis=True)
                self.birth_var_two_d_hist("x_vs_Bz_map_at_"+detector+"_ds_cut", detector, "x", "Bz_map", cuts = "downstream_cut", wild_x_axis=True, wild_y_axis=True)
                self.birth_var_two_d_hist("y_vs_Bz_map_at_"+detector+"_ds_cut", detector, "y", "Bz_map", cuts = "downstream_cut", wild_x_axis=True, wild_y_axis=True)
    ...
    def process_ellipse(self, detector, cut):
        ...
        l_field = (1*r2*Bz/2)  / (1e6/c) # q/q # conv from J mm /s (q/q) -> eV mm /s (sq) -> to MeV/c m 
        ...
        ellipse["l_field"] = l_field # 
        ellipse["l_canon"] = l_kin + l_field + ellipse["l_centre"] # have to add back in mean 
        ellipse["l_canon_minus_mean"] = l_kin + l_field # 
        ellipse["l_twiddle_1"] = ellipse["l_canon"]/2./mu_mass/(emit*1e-3) # with means added back in
        ellipse["l_twiddle_1_minus_mean"] = ellipse["l_canon_minus_mean"]/2./mu_mass/(emit*1e-3) # no messing with mean
        ...
        print 'l_canon_minus_mean', ellipse["l_canon_minus_mean"]
        for axis, index in [("x", 0), ("y", 2)]:
            matrix = numpy.array(rms_ellipse)[index:index+2, index:index+2]
            det = numpy.linalg.det(matrix)
            if det < 1e-9:
