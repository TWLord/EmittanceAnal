...
configdir=config/c$config/$SYSVERS

if [ ! -d $here/config/c$config/$SYSVERS ] ; then

echo "Making new parent directory $here/config/c$config/$SYSVERS "
mkdir -p $here/config/c$config/$SYSVERS
cp -f $here/$templatedir/__init__.py $here/config/c$config/
cp -f $here/$templatedir/__init__.py $here/config/c$config/$SYSVERS/
fi
...
" \
| tee $here/logs/tmp/${rn}_${jobsuffix}_${SYSVERS}.sh
chmod +x $here/logs/tmp/${rn}_${jobsuffix}_${SYSVERS}.sh
bsub -G micegrp -M 20000 -oo $here/logs/${rn}_${jobsuffix}_${SYSVERS}.log -q ${queue} $here/logs/tmp/${rn}_${jobsuffix}_${SYSVERS}.sh
