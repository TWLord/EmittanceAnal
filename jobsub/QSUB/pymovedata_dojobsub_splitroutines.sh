...
# Analysis do_ list:
# "mc" "magnet_alignment" "fractional_emittance" "efficiency" "extrapolation" "globals" "amplitude" "density" "density_rogers" "plots" "cuts_plots" "tof01_weighting" "optics" "data_recorder" "ang_mom_fields"
#for analysis in "mc" "efficiency" "extrapolation" "globals" "amplitude" "density" "plots" "cuts_plots" "optics" "ang_mom_fields"  
...
python $here/bin/run_one_analysis.py config_${config}_${rn}_full.py $index 

cp -rf output/ $here

" \
| tee $here/logs/tmp/run_${runnumber}_${jobsuffix}_movedata_${index}.sh
chmod +x $here/logs/tmp/run_${runnumber}_${jobsuffix}_movedata_${index}.sh
...
qsub -l h_rt=$TIME -q hep.q -wd $here/logs/ $here/logs/tmp/run_${runnumber}_${jobsuffix}_movedata_${index}.sh
