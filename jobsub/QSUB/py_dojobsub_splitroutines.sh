...
echo \$PYTHONPATH
python $here/bin/run_one_analysis.py $configdir/config_${config}_${rn}_full.py ${index} 

" \
| tee $here/logs/tmp/run_${rn}_${jobsuffix}_${index}.sh
chmod +x $here/logs/tmp/run_${rn}_${jobsuffix}_${index}.sh
#bsub -G micegrp -M 20000 -oo $here/logs/run_${rn}_${jobsuffix}.log -q ${queue} $here/logs/tmp/run_${rn}_${jobsuffix}.sh

qsub -l h_rt=$TIME -q hep.q -wd $here/logs/ $here/logs/tmp/run_${rn}_${jobsuffix}_${index}.sh
