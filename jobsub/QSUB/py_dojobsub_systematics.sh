...
sed -e "s/template/${rn}/g" -e "s/ABS/$ABS/g" -e "s/CC/$CC/g" -e "s/VERSION/$VERSION/g" -e "s/SYSTEMATIC/$SYSTEMATIC/g" -e "s/SYSFILE/$SYSFILE/g" -e "s?GEOPATH?$GEOPATH?g" -e "s?$OldDATApath?$DATApath?g" $here/$templatedir/config_${config}_${Optics}.py > $here/$configdir/config_${config}_${rn}_full_${SYSTEMATIC}.py
...
" \
| tee $here/logs/tmp/run_${rn}_${jobsuffix}_${SYSTEMATIC}${VERSION}.sh
chmod +x $here/logs/tmp/run_${rn}_${jobsuffix}_${SYSTEMATIC}${VERSION}.sh
...
qsub -l h_rt=$TIME -q hep.q -wd $here/logs/ $here/logs/tmp/run_${rn}_${jobsuffix}_${SYSTEMATIC}${VERSION}.sh
