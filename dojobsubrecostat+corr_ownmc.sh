...
sed -e "s/template/${rn}/g" -e "s/ABS/$ABS/g" $here/$templatedir/config_6_${Optics}.py > $here/$configdir/config_6_${rn}_full.py
...
python bin/run_one_analysis.py $configdir/config_6_OfficialMC_${runnumber}_reco_stat+corr_ownmc.py

" \
| tee $here/logs/tmp/${runnumber}_recostat+corr_ownmc.sh
chmod +x $here/logs/tmp/${runnumber}_recostat+corr_ownmc.sh
bsub -G micegrp -M 6000 -oo $here/logs/${runnumber}_recostat+corr_ownmc.log -q ${queue} $here/logs/tmp/${runnumber}_recostat+corr_ownmc.sh
